...
//                        Log.i(TAG, "修改后的: $before")
        ...
        return if (result.isEmpty()){
            ...
            RepoResult.RpSuccess(result.map {
                MoveForSelect(moveOrigin = it, moveCE = MoveGenerate.emptyMoveCE, isSelected = false, isMirror = if (mirrorIdList.contains(it.id)) 1 else 0)
            })
        }
    }
    ...
    suspend fun getHandCEWithMirror(startInt: Int?, endInt: Int) :RepoResult<List<MoveForSelect>>{

        var result :List<MoveCE> = listOf()
        val mirrorIdList = mutableListOf<Int>()
        // 先获取到包含起始站架和镜像起始站架的所有招式，这里要按有没有起始站夹限制来区分使用哪一个DAO函数
        if (startInt != null){// 起始站架被限制，我需要按起始站架及镜像来搜索徒手招式
            Log.i(TAG, "getHandCEWithMirror: 查询的起始站架是 ${SideUtil.getSideByInt(startInt)} 镜像站架是 ${SideUtil.getMirrorSide(startInt)} 结束站架是 ${SideUtil.getSideByInt(endInt)}")
            val tempList = moveCEDAO.getHandMoveByStartWithMirror(SideUtil.getSideByInt(startInt), mirrorStartSide = SideUtil.getMirrorSide(startInt))
            // 然后把其中是镜像站架的招式进行全部进行镜像操作
            result = tempList.map {before ->
                if (before.startSide == SideUtil.getMirrorSide(startInt)) {// 只改镜像的招式
//                        Log.i(TAG, "修改前的: $before")
...
//                        Log.i(TAG, "修改后的: $before")
            ...
            val tempEndSide: StandSide = SideUtil.getSideByInt(endInt)
            val tempList = moveCEDAO.getHandMoveByEndWithMirror(tempEndSide, mirrorEndSide = SideUtil.getMirrorSide(endInt))
            result = tempList.map {// 把所有是镜像才能用的招式变化，因为起始站架无限制，所以这个list就已经完全可用了
                if (it.endSide == SideUtil.getMirrorSide(endInt)) {
                    it.startSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(it.startSide))
                    it.endSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(it.endSide))
        ...
        return if (result.isEmpty()){
            RepoResult.RpEmpty("handList CE is empty")
        } else {
            RepoResult.RpSuccess(result.map {
                MoveForSelect(moveOrigin = MoveGenerate.emptyMoveOrigin, moveCE = it, isSelected = false, isMirror = if (mirrorIdList.contains(it.id)) 1 else 0)
            })
        }

    }

    suspend fun getSwordCEWithMirror(startInt: Int?, endInt: Int): RepoResult<List<MoveForSelect>> {
        var result :List<MoveCE> = listOf()
        val swordList = moveCEDAO.getSwordMove()// 先筛选出剑卡中可用的招式
        val mirrorIdList = mutableListOf<Int>()
        // 因为剑卡组有些招式不能镜像使用，所以我需要先筛出可用在剑卡中的招式，然后再根据镜像可用的条件来改变招式数据
        if (startInt != null){// 起始站架被限制，我需要按照起始站架来进行第一次筛选
            val tempStartSide = SideUtil.getSideByInt(startInt)
            result = swordList.filter {ce: MoveCE ->
                if (ce.canOriginSword && !ce.canMirrorSword){// 仅原版可用
                    ce.startSide == tempStartSide
                } else if (!ce.canOriginSword && ce.canMirrorSword){// 仅镜像可用，在确定后要把招式直接修改了，不麻烦在下一层再改
                    if (SideUtil.getMirrorInt(ce.startSide) == startInt){// 如果招式的镜像起始站架等于被限定的起始站架，则相当于这个镜像招式可用，可以修改数据
                        ce.startSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(ce.startSide))
                        ce.endSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(ce.endSide))
                        ce.attackToward = AttackToward.getMirrorToward(ce.attackToward)
                        mirrorIdList.add(ce.id)// 标记镜像使用的招式
                        true
                    } else {
                        false
                    }
                } else if (ce.canOriginSword && ce.canMirrorSword){// 两者皆可使用
                    when (ce.startSide) {
                        tempStartSide -> {// 原版招式的起始站架就已经符合限定的站架，不需要修改
                            true
                        }
            ...
            }.filter { ce: MoveCE -> // 这里已经过了起始站架筛选，且根据不同条件进行了镜像变化，接下来进行结束站架的筛选
                ce.endSide == SideUtil.getSideByInt(endInt)
            }
        } else{// 起始站架没有被限制，需要按结束站架来筛选，但这和上面一样要检查镜像
            val tempEndSide :StandSide = SideUtil.getSideByInt(endInt)
            result = swordList.filter { ce: MoveCE ->
                if (ce.canOriginSword && !ce.canMirrorSword){// 仅原版可用
                    ce.endSide == tempEndSide
                } else if (!ce.canOriginSword && ce.canMirrorSword){// 仅镜像可用，在确定后要把招式直接修改了，不麻烦在下一层再改
                    if (SideUtil.getMirrorInt(ce.endSide) == endInt){// 如果招式的镜像结束站架等于被限定的结束站架，则相当于这个镜像招式可用，可以修改数据
                        ce.startSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(ce.startSide))
                        ce.endSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(ce.endSide))
                        ce.attackToward = AttackToward.getMirrorToward(ce.attackToward)
                        mirrorIdList.add(ce.id)// 标记镜像使用的招式
        ...
        return if (result.isEmpty()){
            RepoResult.RpEmpty("swordList CE is empty")
        } else {
            RepoResult.RpSuccess(result.map {
                MoveForSelect(moveOrigin = MoveGenerate.emptyMoveOrigin, moveCE = it, isSelected = false, isMirror = if (mirrorIdList.contains(it.id)) 1 else 0)
            })
    ...
    /**寻找自选序列可用的招式，起始站架不能和结束站架相同*/
    suspend fun getHandCEOptWithMirror(startInt: Int, endInt: Int) :RepoResult<List<MoveForSelect>>{
        val canNotEndInSide = SideUtil.getSideByInt(startInt)
        val endSide = SideUtil.getSideByInt(endInt)
        var result = listOf<MoveCE>()
        val mirrorIdList = mutableListOf<Int>()
        // 先获取起始针对起始站架筛选的招式，这些招式的起始站架或者是镜像的起始站架都是符合起始站架限制的
        val tempList = moveCEDAO.getHandMoveByStartWithMirror(SideUtil.getSideByInt(startInt), mirrorStartSide = SideUtil.getMirrorSide(startInt))
        result = tempList.map {before ->// 然后把其中是镜像站架的招式进行全部进行镜像操作
        ...
        return if (result.isEmpty()){
            RepoResult.RpEmpty("optHand CE is empty")
        } else {
            RepoResult.RpSuccess(data = result.map {
                MoveForSelect(moveOrigin = MoveGenerate.emptyMoveOrigin, moveCE = it, isSelected = false, isMirror = if (mirrorIdList.contains(it.id)) 1 else 0)
            })
    ...
    suspend fun getSwordCEOptWithMirror(startInt: Int, endInt: Int) :RepoResult<List<MoveForSelect>>{
        var result :List<MoveCE> = listOf()
        val swordList = moveCEDAO.getSwordMove()// 先筛选出剑卡中可用的招式
        val mirrorIdList = mutableListOf<Int>()
        val tempStartSide = SideUtil.getSideByInt(startInt)
