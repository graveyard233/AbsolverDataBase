                     ...
                     private val moveCEDAO: MoveCEDAO)
{
    ...
    suspend fun getHandWithMirror(startInt: Int?,endInt: Int) :RepoResult<List<MoveForSelect>>{
        var result :List<Move> = listOf()
        val mirrorIdList = mutableListOf<Int>()
        // 先获取到包含起始站架和镜像起始站架的所有招式，这里要按有没有起始站夹限制来区分使用哪一个DAO函数
        if (startInt != null){
            Log.i(TAG, "getHandWithMirror: 查询的起始站架是 ${SideUtil.getSideByInt(startInt)} 镜像站架是 ${SideUtil.getMirrorSide(startInt)} 结束站架是 ${SideUtil.getSideByInt(endInt)}")
            val tempList = when(SettingRepository.useWhatDataMod){
                SettingRepository.ORIGIN -> moveOriginDAO.getHandMoveByStartWithMirror(SideUtil.getSideByInt(startInt), mirrorStartSide = SideUtil.getMirrorSide(startInt))
                SettingRepository.CEMOD -> moveCEDAO.getHandMoveByStartWithMirror(SideUtil.getSideByInt(startInt), mirrorStartSide = SideUtil.getMirrorSide(startInt))
                else -> moveOriginDAO.getHandMoveByStartWithMirror(SideUtil.getSideByInt(startInt), mirrorStartSide = SideUtil.getMirrorSide(startInt))
            }
...
//                        Log.i(TAG, "修改后的: $before")
            ...
            val tempList = when(SettingRepository.useWhatDataMod){
                SettingRepository.ORIGIN -> moveOriginDAO.getHandMoveByEndWithMirror(tempEndSide, mirrorEndSide = SideUtil.getMirrorSide(endInt))
                SettingRepository.CEMOD -> moveCEDAO.getHandMoveByEndWithMirror(tempEndSide, mirrorEndSide = SideUtil.getMirrorSide(endInt))
                else -> moveOriginDAO.getHandMoveByEndWithMirror(tempEndSide, mirrorEndSide = SideUtil.getMirrorSide(endInt))
            }
        ...
        return if (result.isEmpty()){
            ...
            RepoResult.RpSuccess(result.map {
                MoveForSelect(move = it, isSelected = false, isMirror = if (mirrorIdList.contains(it.id)) 1 else 0)
            })
        }
    }

    suspend fun getSwordWithMirror(startInt: Int?, endInt: Int): RepoResult<List<MoveForSelect>> {
        var result :List<Move> = listOf()
        val swordList = when(SettingRepository.useWhatDataMod){
            SettingRepository.ORIGIN -> moveOriginDAO.getSwordMove()
            SettingRepository.CEMOD -> moveCEDAO.getSwordMove()
            else -> moveOriginDAO.getSwordMove()
        }
        // 先筛选出剑卡中可用的招式
        val mirrorIdList = mutableListOf<Int>()
        ...
        if (startInt != null){// 起始站架被限制，我需要按照起始站架来进行第一次筛选
            ...
            result = swordList.filter {move: Move ->
                if (move.canOriginSword && !move.canMirrorSword){// 仅原版可用
                    move.startSide == tempStartSide
                } else if (!move.canOriginSword && move.canMirrorSword){// 仅镜像可用，在确定后要把招式直接修改了，不麻烦在下一层再改
                    if (SideUtil.getMirrorInt(move.startSide) == startInt){// 如果招式的镜像起始站架等于被限定的起始站架，则相当于这个镜像招式可用，可以修改数据
                        move.startSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(move.startSide))
                        move.endSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(move.endSide))
                        move.attackToward = AttackToward.getMirrorToward(move.attackToward)
                        mirrorIdList.add(move.id)// 标记镜像使用的招式
                        true
                    } else {
                        false
                    }
                } else if (move.canOriginSword && move.canMirrorSword){// 两者皆可使用
                    when (move.startSide) {
            ...
            }.filter { move: Move -> // 这里已经过了起始站架筛选，且根据不同条件进行了镜像变化，接下来进行结束站架的筛选
                move.endSide == SideUtil.getSideByInt(endInt)
            }
        } else{// 起始站架没有被限制，需要按结束站架来筛选，但这和上面一样要检查镜像
            val tempEndSide :StandSide = SideUtil.getSideByInt(endInt)
            result = swordList.filter { move: Move ->
                if (move.canOriginSword && !move.canMirrorSword){// 仅原版可用
                    move.endSide == tempEndSide
                } else if (!move.canOriginSword && move.canMirrorSword){// 仅镜像可用，在确定后要把招式直接修改了，不麻烦在下一层再改
                    if (SideUtil.getMirrorInt(move.endSide) == endInt){// 如果招式的镜像结束站架等于被限定的结束站架，则相当于这个镜像招式可用，可以修改数据
                        move.startSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(move.startSide))
                        move.endSide = SideUtil.getMirrorSide(SideUtil.getIntBySide(move.endSide))
                        move.attackToward = AttackToward.getMirrorToward(move.attackToward)
                        mirrorIdList.add(move.id)// 标记镜像使用的招式
    ...
    suspend fun getHandOptWithMirror(startInt: Int, endInt: Int) :RepoResult<List<MoveForSelect>>{
        val canNotEndInSide = SideUtil.getSideByInt(startInt)
        val endSide = SideUtil.getSideByInt(endInt)
        var result = listOf<Move>()
        val mirrorIdList = mutableListOf<Int>()
    ...
    suspend fun getSwordOptWithMirror(startInt: Int, endInt: Int) :RepoResult<List<MoveForSelect>>{
        var result :List<Move> = listOf()
        val swordList = when(SettingRepository.useWhatDataMod){
            SettingRepository.ORIGIN -> moveOriginDAO.getSwordMove()
            SettingRepository.CEMOD -> moveCEDAO.getSwordMove()
